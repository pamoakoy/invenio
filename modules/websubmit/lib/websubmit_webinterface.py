...
class WebInterfaceFilesPages(WebInterfaceDirectory):
    ...
    def __call__(self, req, form):
        """Called in case of URLs like /record/123/files or /record/123/extlink?url=.. without
           trailing slash.
        """
        args = wash_urlargd(form, websubmit_templates.files_default_urlargd)
        ln = args['ln']
        ...
        if len(split_uri) > 3 and split_uri[3].startswith('extlink?'):
            ...
            return redirect_to_url(req, '%s/record/%s/extlink/%s' % (CFG_SITE_URL, self.recid, link_url))
        else:
